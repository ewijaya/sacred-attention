...
% ============================================================================
\begin{abstract}
The Catholic Rosary, a prayer of medieval origin comprising repeated vocal prayers organized into decades of ten, has long been recognized for its capacity to induce states of deep contemplation. We develop two complementary mathematical frameworks to analyze the attentional dynamics of Rosary prayer. First, we model the pray-er's cognitive state as a finite absorbing Markov chain with four states---\emph{Distracted}, \emph{Surface Prayer}, \emph{Attentive Prayer}, and \emph{Deep Contemplation} (absorbing)---and derive the fundamental matrix, expected absorption times, and absorption probabilities. We prove that the Rosary's decade structure reduces the expected hitting time to contemplation compared to unstructured repetitive prayer, and that the decade length $L = 10$ is near-optimal under a natural convex tradeoff between attentional reset benefits and interruption costs. Second, we model the brain's attentional system as a damped harmonic oscillator subject to periodic forcing by the Rosary's rhythmic structure, and show that the decade frequency ($\approx 0.1$~Hz) lies near the resonance peak of the attentional oscillator---a finding consistent with Bernardi et al.'s empirical observation that Rosary recitation synchronizes cardiovascular rhythms at precisely this frequency. The synthesis of these models suggests that the Rosary's structure is not merely traditional but, in a precise mathematical sense, \emph{tuned} to the architecture of human attention.
\end{abstract}
...
\node[right,font=\footnotesize] at (axis cs:0.66,26) {Rosary $\omega_b$};

% Vertical line at resonant frequency
\draw[red,dotted,thin] (axis cs:0.591,0) -- (axis cs:0.591,29.9);
\node[left,font=\footnotesize,red] at (axis cs:0.56,32) {$\omega_r$};

% Half-power bandwidth arrows
\draw[<->,gray,thin] (axis cs:0.44,14.95) -- (axis cs:0.74,14.95);
\node[above,font=\tiny,gray] at (axis cs:0.59,15.5) {bandwidth $\approx 2\gamma$};
...
\begin{axis}[
  ...
  title={Simulated Attention Trajectories},
  every axis title/.style={font=\small},
  ytick={1,2,3,4},
  yticklabels={$D$,$S$,$A$,$C$}
]
...
\draw[gray,dotted,thin] (axis cs:10,0) -- (axis cs:10,4.2);
\draw[gray,dotted,thin] (axis cs:20,0) -- (axis cs:20,4.2);
\draw[gray,dotted,thin] (axis cs:30,0) -- (axis cs:30,4.2);
\draw[gray,dotted,thin] (axis cs:40,0) -- (axis cs:40,4.2);
\draw[gray,dotted,thin] (axis cs:50,0) -- (axis cs:50,4.2);

% Label decade boundaries
\node[above,font=\tiny,gray] at (axis cs:10,4.15) {I};
\node[above,font=\tiny,gray] at (axis cs:20,4.15) {II};
\node[above,font=\tiny,gray] at (axis cs:30,4.15) {III};
\node[above,font=\tiny,gray] at (axis cs:40,4.15) {IV};
\node[above,font=\tiny,gray] at (axis cs:50,4.15) {V};
